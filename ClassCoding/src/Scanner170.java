...
/* This code has been modified by Dr. Yacobellis for COMP 170, Intro to OO.
 * hasNextChar and nextChar were added to return the next character, and
 * versions hasNextCharX and nextCharX to return even whitespace; also
 * "aliases" for [has]nextLine and [has]next were added: [has]nextString,
 * [has]nextWord, and [has]nextToken. Finally, an annoyance has been fixed:
 * when a next... call reads a token at the end of a line and nextLine is
 * called immediately afterward, the original Scanner returns the empty String
 * rather than reading the following line (typically this happens with user
 * input, leading to the surprising result that the Scanner does not receive
 * and return the user's input). The current version detects this case and
 * instead skips the line ending and reads the next full line.
 * One more minor change: System.in is not allowed to be closed.
 */
 ...
 * regular expressions.
 *
 * <p>A {@code Scanner170} breaks its input into tokens using a
 ...
 *
 * <p>When a {@code Scanner170} is closed, it will close its input source
 * if the source implements the {@link java.io.Closeable} interface.
 *
 * <p>A {@code Scanner170} is not safe for multithreaded use without
 * external synchronization.
 *
 * <p>Unless otherwise mentioned, passing a {@code null} parameter into
 * any method of a {@code Scanner170} will cause a
 ...
 * @since   1.5
 */
public final class Scanner170 implements Iterator<String>, Closeable {

    // boolean to say if the previous Scanner170 method call was next... **NEW**
    private boolean nextCalled = false; // initially false

    // boolean to say if this Scanner170 was opened to System.In **NEW**
    private boolean SystemIn = false; // initially false

    // static Scanner170 variable to hold the single System.In Scanner170 **NEW**
    private static Scanner170 SysInScanner = null; // none yet ...
    ...
    // A pattern for a single character **NEW**
    private final static Pattern CHAR_PATTERN = Pattern.compile(".");

    // A pattern for a single character including whitespace characters **NEW**
    private final static Pattern CHAR_PATTERN_X = Pattern.compile(".");
    ...
    /**
     * Constructs a {@code Scanner170} that returns values scanned
     * from the specified source delimited by the specified pattern.
     *
     * @param source A character source implementing the Readable interface
     * @param pattern A delimiting pattern
     */
    private Scanner170(Readable source, Pattern pattern) {
    ...
    /**
     * A new method allowing another constructor to call this constructor's logic
     */

    private void readable(Readable source, Pattern pattern) {
        assert source != null : "source should not be null";
        assert pattern != null : "pattern should not be null";
        this.source = source;
        delimPattern = pattern;
     ...
     * Constructs a {@code Scanner170} that returns values scanned
     * from the specified source delimited by the specified pattern.
     * Also checks to see if the InputStream is System.in
     *
     * @param source A character source implementing the Readable interface
     * @param pattern A delimiting pattern
     */
    private Scanner170(Readable source, Pattern pattern, InputStream is) {
        inputStream(source, pattern, is);
		/*
        assert source != null : "source should not be null";
        assert pattern != null : "pattern should not be null";
		if (is == System.in) // **NEW**
			SystemIn = true; // this is System.in - don't allow closing it!
		else
			SystemIn = false;
        this.source = source;
        delimPattern = pattern;
    ...
    /**
     * private method added to allow processing System.in from another constructor
     */

    private void inputStream(Readable source, Pattern pattern, InputStream is) {
        assert source != null : "source should not be null";
        assert pattern != null : "pattern should not be null";
        if (is == System.in) { // **NEW**
            SystemIn = true; // this is System.in - don't allow closing it!
            if (SysInScanner == null) // if not yet initialized ...
                SysInScanner = this; // initialize the single System.In Scanner170
        }
        else
            SystemIn = false;
        this.source = source;
    ...
    /**
     * Constructs a new {@code Scanner170} that produces values scanned
     * from the specified source.
     *
     * @param  source A character source implementing the {@link Readable}
     *         interface
     */
    public Scanner170(Readable source) {
     ...
     */
    public Scanner170(InputStream source) {
        this(new InputStreamReader(source), WHITESPACE_PATTERN, source);
    }
     ...
     *         does not exist
     */
    public Scanner170(InputStream source, String charsetName) {
     ...
     * @since  10
     */
    public Scanner170(InputStream source, Charset charset) {
        this(makeReadable(Objects.requireNonNull(source, "source"), charset),
                WHITESPACE_PATTERN, source);
    }
     ...
     */
    public Scanner170(File source) throws FileNotFoundException {
        this((ReadableByteChannel)(new FileInputStream(source).getChannel()));
    }

    /**
     * New method to allow another constructor to call this logic
     */

    private void file(File source) {
        try {
            readable(makeReadable(Objects.requireNonNull((ReadableByteChannel)(new FileInputStream(source).getChannel()), "source")),
                    WHITESPACE_PATTERN);
        }
        catch (FileNotFoundException e) {
            throw new RuntimeException("a file error occurred for " + source);
        }
    }
     ...
     *         not found
     */
    public Scanner170(File source, String charsetName)
     ...
     * @since  10
     */
    public Scanner170(File source, Charset charset) throws IOException {
        this(Objects.requireNonNull(source), charset.newDecoder());
    }

    private Scanner170(File source, CharsetDecoder dec)
            throws FileNotFoundException
    {
     ...
     * @since   1.7
     */
    public Scanner170(Path source, String charsetName) throws IOException {
     ...
     * @since   10
     */
    public Scanner170(Path source, Charset charset)  throws IOException {
     ...
     */
    public Scanner170(String source) {
        // modified to handle System.in/Keyboard/Console and file: **NEW**
        if (source.equals("System.In")
                || source.equals("Console")
                || source.equals("Keyboard")) // **NEW**
            inputStream(new InputStreamReader(System.in), WHITESPACE_PATTERN, System.in);
            // process aliases for System.in
        else if (source.indexOf("file: ") == 0) // this is a file reference **NEW**
        {
            String file = source.substring(6).trim(); // get the file reference
            file(new File(file));
        }
        else { // **NEW**
            readable(new StringReader(source), WHITESPACE_PATTERN);
        }
    }

    /**
     * New constructor that provides an alias for new Scanner170(System.in)
     */

    public Scanner170() { // **NEW**
        this(System.in);
    }
     ...
     */
    public Scanner170(ReadableByteChannel source) {
     ...
     *         does not exist
     */
    public Scanner170(ReadableByteChannel source, String charsetName) {
     ...
     * @since 10
     */
    public Scanner170(ReadableByteChannel source, Charset charset) {
        this(makeReadable(Objects.requireNonNull(source, "source"), charset),
                WHITESPACE_PATTERN);
    }
    ...
    private String getCompleteTokenInBuffer(Pattern pattern) {
        matchValid = false;
        if (pattern != CHAR_PATTERN_X) // only skip delims if not looking for a whitespace character **NEW**
        {
            // Skip delims first
            matcher.usePattern(delimPattern);
        ...
        if (pattern == CHAR_PATTERN || pattern == CHAR_PATTERN_X) // looking for a character? **NEW**
        {
            char c = buf.get(position); // get the first one in the buffer
            position++; // indicate that we've moved past it
            return ""+c; // and return it as a String
        }
     ...
     */
    public void close() {
        if (closed || SystemIn) // don't allow closing System.in **NEW**
            return;
     ...
     * {@code Scanner170}'s underlying {@code Readable}. This method
    ...
    /**
     * Returns the {@code Pattern} this {@code Scanner170} is currently
     ...
     * @return this scanner
     */
    public Scanner170 useDelimiter(Pattern pattern) {
        modCount++;
        delimPattern = pattern;
        return this;
    }
     ...
     * @return this scanner
     */
    public Scanner170 useDelimiter(String pattern) {
        modCount++;
     ...
     * @return this scanner
     */
    public Scanner170 useLocale(Locale locale) {
        ...
        if (nf instanceof DecimalFormat) {
			...
			*/
            throw new RuntimeException("LocaleProviderAdapter not supported");
        }
     ...
     */
    public Scanner170 useRadix(int radix) {
     ...
     * not successful.
     *
     * <p>The various {@code next} methods of {@code Scanner170}
    ...
    /**
     * <p>Returns the string representation of this {@code Scanner170}. The
     * string representation of a {@code Scanner170} contains information
    ...
    public boolean hasNext() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNext();
        ensureOpen();
        saveState();
        modCount++;
        while (!sourceClosed) {
    ...
    public boolean hasNextWord() { // **NEW**
        return hasNext();
    }

    public boolean hasNextToken() { // **NEW**
        return hasNext();
    }
    ...
    public String next() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.next();

        nextCalled = true; // a next... method was called **NEW**
    ...
    public boolean hasNextLine() {
        if (SystemIn && this != SysInScanner) { // **NEW**
            System.out.println("SystemIn detected");
            return SysInScanner.hasNextLine();
        }
    ...
    public String nextLine() {
        ...
        if (hasNextPattern == linePattern()) {
            ...
            if (nextCalled) { // was there a preceeding next...? **NEW**
                nextCalled = false; // nextLine clears that flag
                if (result.length() == 0) { // if there's nothing left on the line (newline ended the next...)
                    clearCaches(); // clear the caches
                    return nextLine(); // and read the user's input instead of returning the empty String
                }
				/* we only want to read ahead if the previous next... was terminated by a newline character
					Matcher nextMatcher = delimPattern.matcher(result);
					nextMatcher.region(0, result.length());
					if (nextMatcher.lookingAt() && nextMatcher.end() == result.length()) { // if there are only delimeters left
						clearCaches(); // clear the caches
						return nextLine(); // and read the user's input instead of returning the delimeters
					}
					else if (nextMatcher.lookingAt()) {// if there are leading delimiters but also more
						result = result.substring(nextMatcher.end()); // remove them before returning the line
					}
				*/
            }
            return result;
        ...
        if (result == null)
            ...
            if (nextCalled) { // was there a preceeding next...? **NEW**
                nextCalled = false; // nextLine clears that flag
                if (result.length() == 0) // if there's nothing left on the line (newline ended the next...)
                    return nextLine(); //  // read the user's input instead of returning the empty String
				/* we only want to read ahead if the previous next... was terminated by a newline character
					Matcher nextMatcher = delimPattern.matcher(result);
					nextMatcher.region(0, result.length());
					if (nextMatcher.lookingAt() && nextMatcher.end() == result.length()) // if there are only delimeters left
						return nextLine(); // read the user's input instead of returning them
					else if (nextMatcher.lookingAt()) { // if there are leading delimiters but also more
						result = result.substring(nextMatcher.end()); // remove them before returning the line
					}
				*/
    ...
    /**
     * hasNextChar() - true if there is a next character available to read
     */

    public boolean hasNextChar() { // **NEW**
        return hasNext();
        // true if there is a next token available (characters will be taken from it)
    }

    /**
     * nextChar() - returns the next available character, if any
     */

    public char nextChar() { // **NEW**
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.nextChar();

        return next(CHAR_PATTERN).charAt(0);
    }

    public boolean hasNextCharX() { // **NEW**
        return hasNext();
        // true if there is a next character available
    }

    /**
     * nextCharX() - returns the next available character, if any, including whitespace characters
     */

    public char nextCharX() { // **NEW**
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.nextCharX();

        return next(CHAR_PATTERN_X).charAt(0);
    }
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public Scanner170 skip(Pattern pattern) {
        ensureOpen();
        if (pattern == null)
            throw new NullPointerException();
        clearCaches();
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public Scanner170 skip(String pattern) {
    ...
    public boolean hasNextBoolean() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextBoolean();
    ...
    public boolean nextBoolean() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.nextBoolean();
    ...
    public boolean hasNextByte() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextByte();
    ...
    public boolean hasNextShort() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextShort();
    ...
    public boolean hasNextInt() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextInt();
    ...
    public boolean hasNextInt(int radix) {
        setRadix(radix);
        boolean result = result = hasNext(integerPattern());
        if (result) { // Cache it
            try {
                String s = (matcher.group(SIMPLE_GROUP_INDEX) == null) ?
                        processIntegerToken(hasNextResult) :
    ...
    public int nextInt(int radix) {
        ...
        try {
            ...
            position = matcher.start(); // don't skip bad token or empty nextChar buffer **NEW**
            throw new InputMismatchException(nfe.getMessage());
        }
    }
    ...
    public boolean hasNextLong() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextLong();
    ...
    public boolean hasNextFloat() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextFloat();
    ...
    public boolean hasNextDouble() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.hasNextDouble();
    ...
    public double nextDouble() {
        if (SystemIn && this != SysInScanner) // **NEW**
            return SysInScanner.nextDouble();
